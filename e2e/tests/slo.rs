...
// Calculate minimum requests rate
// sum(rate(..[1m])) - rps
// (sum(..)) > 10 to filter out insufficient load
// min_over_time((sum(..)>10)[1h:5m]) - minumum rps per 5 mins for last hour
#[test_case("min_over_time((sum(rate(cincinnati_pe_graph_incoming_requests_total[1m])) > 10)[1h:5m])" => is greater_than(100.0); "OSUS can handle at least 100rps")]
